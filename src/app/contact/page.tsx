...
function ContactFormContent() {
  ...
  useEffect(() => {
    ...
    // Fetch dynamic services added in Admin & exclude Vending Machine
    fetch("/api/services", { cache: "no-store" })
      ...
      .then((data) => {
        ...
        const nonVendingServices = rawList.filter(
          (s: { id: string; title: string }) =>
            s.id !== "SRV-01" &&
            !String(s.title || "").toLowerCase().includes("вендинг")
        );

        setServicesList(nonVendingServices);

        const urlService = searchParams?.get("service");
        if (urlService) {
          setService(urlService);
        } else if (nonVendingServices.length > 0) {
          setService(nonVendingServices[0].title);
        } else {
          setService("Друга услуга / Общо запитване");
        }
      })
      .catch(() => {});
  }, [searchParams]);
              ...
              <div className="relative">
                ...
                >
                  {servicesList.length > 0 ? (
                    ...
                    <>
                      <option value="ДЪРВЕН печат">ДЪРВЕН печат</option>
                      ...
                      <option value="ПЕРСОНАЛИЗИРАНИ КАРТИЧКИ">ПЕРСОНАЛИЗИРАНИ КАРТИЧКИ</option>
